    ...
    : IItemRepository, IItemQueryHelpers
{
    ...
    private bool? _usePostgreSqlQueryOptimizations;
    ...
    private bool UsePostgreSqlQueryOptimizations()
    {
        if (!_usePostgreSqlQueryOptimizations.HasValue)
        {
            using var context = _dbProvider.CreateDbContext();
            _usePostgreSqlQueryOptimizations = string.Equals(
                context.Database.ProviderName,
                "Npgsql.EntityFrameworkCore.PostgreSQL",
                StringComparison.Ordinal);
        }

        return _usePostgreSqlQueryOptimizations.Value;
    }
}
