...
public sealed class PostgreSqlHotCacheAdministration(NpgsqlDataSource dataSource, IHotCacheCoordinator coordinator) : IHotCacheAdministration
{
    ...
    public async Task QueueActionAsync(HotCacheAction action, CancellationToken cancellationToken)
    {
        ...
        const string sql = "WITH target AS (SELECT item_id FROM hot_cache_jobs WHERE id=@id AND @action='evict'), cleared AS (DELETE FROM hot_cache_interests i USING target t WHERE i.item_id=t.item_id) UPDATE hot_cache_jobs j SET kind=CASE WHEN @action='promote' THEN 'promotion' WHEN @action='evict' THEN 'eviction' ELSE j.kind END,state='pending',priority=CASE WHEN @action='evict' AND @id IS NOT NULL THEN 0 ELSE j.priority END,attempts=0,last_error=NULL,lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE (@id IS NULL OR j.id=@id) AND (@action <> 'promote' OR j.state <> 'running') AND (@action <> 'retry' OR j.state='failed') AND (@action <> 'evict' OR (j.state='completed' AND j.hot_path IS NOT NULL AND NOT j.is_active AND NOT j.is_pinned AND (@id IS NOT NULL OR j.priority <= 0) AND NOT EXISTS(SELECT 1 FROM hot_cache_playback_leases l WHERE l.item_id=j.item_id AND l.expires_at_utc>now())))";
        await using var command = dataSource.CreateCommand(sql);
