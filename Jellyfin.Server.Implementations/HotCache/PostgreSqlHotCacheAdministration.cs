...
public sealed class PostgreSqlHotCacheAdministration(NpgsqlDataSource dataSource, IHotCacheCoordinator coordinator) : IHotCacheAdministration
{
    ...
    {
        const string sql = "WITH states(state) AS (VALUES ('queued'),('copying'),('evicting'),('copied'),('evicted'),('failed')), totals AS (SELECT CASE WHEN state='pending' THEN 'queued' WHEN state='running' AND kind='promotion' THEN 'copying' WHEN state='running' AND kind='eviction' THEN 'evicting' WHEN state='completed' AND hot_path IS NOT NULL THEN 'copied' WHEN state='completed' AND hot_path IS NULL THEN 'evicted' ELSE 'failed' END AS state,COUNT(*) AS count,COALESCE(SUM(source_length),0) AS bytes FROM hot_cache_jobs GROUP BY 1) SELECT states.state,COALESCE(totals.count,0),COALESCE(totals.bytes,0) FROM states LEFT JOIN totals ON totals.state=states.state ORDER BY states.state";
    ...
    {
        const string sql = "SELECT j.id,COALESCE(j.series_name,'Uncatalogued'),COALESCE(j.episode_name,j.id::text),COALESCE(i.reason,''),COALESCE(i.users,0),j.priority,j.source_length,COALESCE(j.backend,''),j.created_at,j.updated_at,CASE WHEN j.state='pending' THEN 'queued' WHEN j.state='running' AND j.kind='promotion' THEN 'copying' WHEN j.state='running' AND j.kind='eviction' THEN 'evicting' WHEN j.state='completed' AND j.hot_path IS NOT NULL THEN 'copied' WHEN j.state='completed' AND j.hot_path IS NULL THEN 'evicted' ELSE 'failed' END FROM hot_cache_jobs j LEFT JOIN (SELECT item_id,STRING_AGG(DISTINCT reason,',' ) reason,COUNT(DISTINCT user_id) users FROM hot_cache_interests WHERE expires_at_utc > now() GROUP BY item_id) i ON i.item_id=j.item_id WHERE j.hot_path IS NOT NULL OR j.state IN ('pending','running') OR i.item_id IS NOT NULL ORDER BY COALESCE(j.series_name,''),COALESCE(j.episode_name,''),j.id LIMIT 500";
