...
public sealed class PostgreSqlHotCacheAdministration(NpgsqlDataSource dataSource, IHotCacheCoordinator coordinator) : IHotCacheAdministration
{
    ...
    {
        if (action.Kind is not ("promote" or "evict" or "retry" or "reconcile")) throw new ArgumentException("Unknown action kind.", nameof(action));
        if (action.Kind == "evict" && action.ItemId is null && !action.ConfirmBulkEviction) throw new ArgumentException("Bulk eviction requires confirmation.", nameof(action));
        if (action.Kind is not "reconcile" && action.ItemId is null) throw new ArgumentException("An inventory item is required.", nameof(action));
        ...
        if (action.ItemId is not null)
        {
            ...
            if (!(bool)(await check.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false) ?? false)) throw new KeyNotFoundException("Inventory item was not found.");
        }
    ...
    private async Task<IReadOnlyList<HotCacheBackendStatus>> ReadBackendsAsync(CancellationToken ct)
    {
        ...
        var results = new List<HotCacheBackendStatus>(); await using var command = dataSource.CreateCommand(sql); await using var reader = await command.ExecuteReaderAsync(ct).ConfigureAwait(false);
        while (await reader.ReadAsync(ct).ConfigureAwait(false)) { var at = reader.GetDateTime(6); results.Add(new(reader.GetString(0), reader.GetBoolean(1), reader.GetBoolean(2), at < DateTime.UtcNow.AddMinutes(-2), reader.GetInt64(3), reader.GetInt64(4), reader.GetInt64(5), at)); }
        return results;
    }
    ...
    private async Task<IReadOnlyList<HotCacheQueueSummary>> ReadQueueAsync(CancellationToken ct)
    {
        ...
        var results = new List<HotCacheQueueSummary>(); await using var command = dataSource.CreateCommand(sql); await using var reader = await command.ExecuteReaderAsync(ct).ConfigureAwait(false);
        while (await reader.ReadAsync(ct).ConfigureAwait(false)) results.Add(new(reader.GetString(0), reader.GetInt64(1), reader.GetInt64(2)));
        return results;
    }
    ...
    private async Task<IReadOnlyList<HotCacheInventoryItem>> ReadInventoryAsync(string backend, CancellationToken ct)
    {
        ...
        var results = new List<HotCacheInventoryItem>(); await using var command = dataSource.CreateCommand(sql); command.Parameters.AddWithValue("backend", backend); await using var reader = await command.ExecuteReaderAsync(ct).ConfigureAwait(false);
        while (await reader.ReadAsync(ct).ConfigureAwait(false)) results.Add(new(reader.GetGuid(0), "Uncatalogued", reader.GetGuid(0).ToString(), reader.GetString(1), checked((int)reader.GetInt64(2)), reader.GetInt32(3), reader.GetInt64(4), reader.GetString(5), reader.GetDateTime(6), reader.GetDateTime(7), reader.GetString(8)));
        return results;
    }
    ...
    private async Task<IReadOnlyList<HotCacheHistoryEntry>> ReadHistoryAsync(string? kind, CancellationToken ct)
    {
        ...
        var results = new List<HotCacheHistoryEntry>(); await using var command = dataSource.CreateCommand(sql); command.Parameters.AddWithValue("kind", (object?)kind ?? DBNull.Value); await using var reader = await command.ExecuteReaderAsync(ct).ConfigureAwait(false);
        while (await reader.ReadAsync(ct).ConfigureAwait(false)) results.Add(new(reader.GetInt64(0), reader.GetString(1), reader.GetString(2), reader.GetDateTime(3)));
        return results;
    }

    private async Task HistoryAsync(string kind, string detail, CancellationToken ct)
    { await using var command = dataSource.CreateCommand("INSERT INTO hot_cache_admin_history(kind,detail) VALUES(@kind,@detail)"); command.Parameters.AddWithValue("kind", kind); command.Parameters.AddWithValue("detail", detail); await command.ExecuteNonQueryAsync(ct).ConfigureAwait(false); }

    private static void Validate(HotCacheSettings settings)
    { if (settings.Backend is not ("unraid-temp" or "cephfs")) throw new ArgumentException("Unknown backend."); if (settings.LowWatermark <= 0 || settings.HighWatermark >= 1 || settings.LowWatermark >= settings.HighWatermark) throw new ArgumentException("Watermarks must be between zero and one, with low below high."); }
}
