...
public interface IFileOperations
{
    ...
    Task CopyAsync(string source, string destination, Action<long> progress, CancellationToken cancellationToken);
...
public sealed class PhysicalFileOperations : IFileOperations
{
    ...
    public async Task CopyAsync(string source, string destination, Action<long> progress, CancellationToken ct)
    {
        ...
        while ((read = await input.ReadAsync(buffer, ct).ConfigureAwait(false)) != 0) { await output.WriteAsync(buffer.AsMemory(0, read), ct).ConfigureAwait(false); progress(copied += read); }
...
public sealed class HotCacheWorker
{
    ...
    public HotCacheWorker(IHotCacheJobStore store, IFileOperations files, HotCacheOptions options, ILogger<HotCacheWorker> logger) => (_store, _files, _options, _logger) = (store, files, options, logger);
    ...
    {
        try { if (job.Kind == HotCacheJobKind.Promotion) await PromoteAsync(job, workerId, ct).ConfigureAwait(false); else await EvictAsync(job, workerId, ct).ConfigureAwait(false); await _store.CompleteAsync(job.Id, workerId, ct).ConfigureAwait(false); }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or OperationCanceledException) { await _store.FailAsync(job.Id, workerId, Bound(ex.Message), CancellationToken.None).ConfigureAwait(false); _logger.LogWarning(ex, "Hot-cache job {JobId} degraded to cold playback", job.Id); }
    }
    private async Task PromoteAsync(HotCacheJob job, string workerId, CancellationToken ct)
    {
        ...
        _files.CreateDirectory(Path.GetDirectoryName(target)!); var partial = Path.Combine(Path.GetDirectoryName(target)!, $".{Path.GetFileName(target)}.{job.Id:N}.partial");
        try { await _files.CopyAsync(source, partial, bytes => _store.ProgressAsync(job.Id, workerId, bytes, CancellationToken.None).GetAwaiter().GetResult(), ct).ConfigureAwait(false); var after = _files.GetFileInfo(source); if (after.Length != before.Length || after.LastWriteTimeUtc != before.LastWriteTimeUtc) throw new IOException("Source changed during promotion."); _files.MoveNoReplace(partial, target); await _store.EventAsync(job.Id, "published", target, ct).ConfigureAwait(false); }
        catch { if (_files.FileExists(partial)) _files.Delete(partial); throw; }
    }
