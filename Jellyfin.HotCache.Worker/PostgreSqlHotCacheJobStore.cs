...
public sealed class PostgreSqlHotCacheJobStore(NpgsqlDataSource dataSource) : IHotCacheJobStore
{
    ...
    { const string sql="SELECT count(*) FILTER (WHERE state IN ('pending','running')), COALESCE(EXTRACT(EPOCH FROM now()-min(created_at) FILTER (WHERE state IN ('pending','running'))),0), COALESCE(EXTRACT(EPOCH FROM now()-min(updated_at) FILTER (WHERE state='running' AND lease_expires_at>now())),0) FROM hot_cache_jobs"; await using var cmd=dataSource.CreateCommand(sql); await using var r=await cmd.ExecuteReaderAsync(ct).ConfigureAwait(false); await r.ReadAsync(ct).ConfigureAwait(false); return new(r.GetInt64(0), TimeSpan.FromSeconds(r.GetDouble(1)), TimeSpan.FromSeconds(r.GetDouble(2))); }
