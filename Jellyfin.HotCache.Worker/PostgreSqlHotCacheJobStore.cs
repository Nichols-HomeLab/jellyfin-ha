...
public sealed class PostgreSqlHotCacheJobStore(NpgsqlDataSource dataSource) : IHotCacheJobStore
{
    ...
    public Task CompleteAsync(Guid id, string owner, string? hotPath, CancellationToken ct) => ExecuteOwnedAsync("UPDATE hot_cache_jobs SET state='completed',is_copying=false,hot_path=CASE WHEN kind='eviction' THEN NULL ELSE COALESCE(@hotPath,hot_path) END,lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE id=@id AND lease_owner=@owner AND lease_expires_at>now()", id, owner, ct, ("hotPath", (object?)hotPath ?? DBNull.Value));
