...
public class DtoServiceImageInheritanceTests
{
    ...
    public void GetBaseItemDtos_Items_ResolvePeopleFromBatch_WithoutPerItemLookup()
    {
        ...
        // v12 resolves distinct Person entities through the paged people query to attach images.
        libraryManager
            .Setup(x => x.GetPeopleItems(It.IsAny<InternalPeopleQuery>()))
            .Returns(new QueryResult<BaseItem>
            {
                Items = [new Person { Id = Guid.NewGuid(), Name = "Some Actor" }],
                TotalRecordCount = 1
            });
