...
namespace Emby.Server.Implementations.Library
{
    ...
    public class LibraryManager : ILibraryManager
    {
        ...
        {
            _catalogOwnership.RequireCatalogWriteToken();
            _persistenceService.DeleteItem([.. items.Select(item => item.Id)]);
        ...
        public void UpsertLinkedChild(Guid parentId, Guid childId, MediaBrowser.Controller.Entities.LinkedChildType childType)
        {
            ...
            _catalogChangeNotifier.Publish(CatalogChange.Local(CatalogChangeKind.Updated, parentId, GetItemById(parentId)?.ParentId ?? Guid.Empty));
        }
        ...
        public void CreateItems(IReadOnlyList<BaseItem> items, BaseItem? parent, CancellationToken cancellationToken)
        {
            ...
            _persistenceService.SaveItems(allItems, catalogWrite.Token);
        ...
        public QueryResult<BaseItem> GetPeopleItems(InternalPeopleQuery query)
        {
            ...
            var names = queryResult.Items.Select(person => person.Name).ToList();
            if (names.Count == 0)
            {
                return new QueryResult<BaseItem> { StartIndex = queryResult.StartIndex, TotalRecordCount = queryResult.TotalRecordCount, Items = [] };
            }
        ...
        public async Task RerouteLinkedChildReferencesAsync(Guid fromChildId, Guid toChildId)
        {
            ...
            foreach (var parentId in affectedParentIds)
            {
                if (GetItemById(parentId) is Folder parent)
                {
                    ...
                    _catalogChangeNotifier.Publish(CatalogChange.Local(CatalogChangeKind.Updated, parent.Id, parent.ParentId));
                }
            }
        }
