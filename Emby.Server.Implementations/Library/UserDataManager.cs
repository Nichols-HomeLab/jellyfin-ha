    ...
    public sealed class UserDataManager : IUserDataManager, IDisposable
    {
        ...
        public Dictionary<Guid, UserItemData> GetUserDataBatch(IReadOnlyList<BaseItem> items, User user)
        {
            ...
            foreach (var item in items)
            {
                ...
                {
                    // A remote invalidation must reload persisted data rather than the stale
                    // navigation rows retained on a cached BaseItem.
        ...
        {
            ArgumentNullException.ThrowIfNull(user);
            return GetUserDataBatch([item], user)[item.Id];
        }
