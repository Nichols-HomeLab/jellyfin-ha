          ...
          <div data-role="content">
            <div class="content-primary hc-shell">
              ...
              (()=>{
                ...
                const viewState={inventoryRendered:false,openSeries:new Set()};
                ...
                const renderInventory=inventory=>{const target=byId('hotCacheInventory'),picker=byId('hotCacheItem');const selectedItem=picker.value;if(viewState.inventoryRendered)viewState.openSeries=new Set(Array.from(target.querySelectorAll('.hc-series[open]')).map(details=>details.dataset.series));picker.replaceChildren(new Option('All eligible items',''));inventory.forEach(item=>picker.add(new Option(item.seriesName+' — '+item.episode,item.itemId)));if(inventory.some(item=>item.itemId===selectedItem))picker.value=selectedItem;if(!inventory.length){empty(target,'No cache candidates yet. Start or resume an episode, then refresh.');viewState.inventoryRendered=true;return;}const groups=new Map();inventory.forEach(item=>{if(!groups.has(item.seriesName))groups.set(item.seriesName,[]);groups.get(item.seriesName).push(item);});target.replaceChildren(...Array.from(groups.entries()).sort(([left],[right])=>left.localeCompare(right)).map(([series,items],index)=>{const details=element('details','hc-series');details.dataset.series=series;details.open=viewState.inventoryRendered?viewState.openSeries.has(series):index===0;const summary=document.createElement('summary'),summaryText=element('strong','',series),total=items.reduce((sum,item)=>sum+item.sizeBytes,0);summary.append(summaryText,element('span','hc-series-count',items.length+' '+(items.length===1?'episode':'episodes')+' · '+formatBytes(total)));const body=document.createElement('div');table(body,['Episode','State','Interest','Users','Priority','Size','Backend','Updated'],items.map(item=>[item.episode,stateBadge(item.state),item.reason||'—',String(item.interestedUsers),String(item.priority),formatBytes(item.sizeBytes),item.backend||'—',formatDate(item.updatedAtUtc)]));details.append(summary,body);return details;}));viewState.inventoryRendered=true;};
