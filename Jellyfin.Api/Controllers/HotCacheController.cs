        ...
        <div id="hotCachePage" data-role="page" class="page type-interior pluginConfigurationPage" data-require="emby-button,emby-checkbox,emby-input,emby-select">
          ...
          <div data-role="content">
            <div class="content-primary hc-shell">
              ...
              <section class="hc-panel hc-section">
                <div class="hc-panel-head"><div><h2>Inventory by series</h2><p class="hc-panel-copy">Playback and lookahead candidates grouped for quick inspection.</p></div></div>
              ...
              (()=>{
                ...
                const viewState={inventoryRendered:false,openSeries:new Set(),scrollBySeries:new Map()};
                ...
                const renderSummary=snapshot=>{
                  ...
                  byId('hotCacheSummaryInventory').textContent=new Intl.NumberFormat().format(inventory.length);byId('hotCacheSummarySeries').textContent=series+' series';byId('hotCacheSummaryFailures').textContent=new Intl.NumberFormat().format(failures);
                };

                const renderBackends=backends=>{const target=byId('hotCacheBackends');if(!backends.length){empty(target,'No worker has reported a backend observation yet.');return;}target.replaceChildren(...backends.map(backend=>{const card=element('article','hc-backend'),title=element('div','hc-backend-title'),status=!backend.mounted?['Unmounted','bad']:backend.stale?['Stale','warn']:backend.healthy?['Healthy','good']:['Unhealthy','bad'],percent=backend.totalBytes?Math.min(100,Math.round((backend.usedBytes/backend.totalBytes)*100)):0,meter=element('div','hc-meter'),fill=document.createElement('span');title.append(element('strong','',backend.name),element('span','hc-badge '+status[1],status[0]));fill.style.width=percent+'%';meter.append(fill);const capacity=element('div','hc-meta');capacity.append(element('span','',formatBytes(backend.usedBytes)+' used'),element('span','',percent+'%'));const observed=element('div','hc-meta');observed.style.marginTop='.45rem';observed.append(element('span','',formatBytes(backend.availableBytes)+' available'),element('span','',formatDate(backend.observedAtUtc)));card.append(title,meter,capacity,observed);return card;}));};
                const renderQueue=queue=>{const target=byId('hotCacheQueue'),order=['queued','copying','evicting','copied','evicted','failed'];target.replaceChildren(...order.map(state=>{const item=queueValue(queue,state),card=element('article','hc-queue-card');card.append(stateBadge(state),element('strong','',new Intl.NumberFormat().format(item.count)),element('span','hc-stat-note',formatBytes(item.bytes)));return card;}));};
                const renderInventory=inventory=>{const target=byId('hotCacheInventory'),picker=byId('hotCacheItem');const selectedItem=picker.value;if(viewState.inventoryRendered){viewState.openSeries=new Set(Array.from(target.querySelectorAll('.hc-series[open]')).map(details=>details.dataset.series));viewState.scrollBySeries=new Map(Array.from(target.querySelectorAll('.hc-series')).map(details=>[details.dataset.series,details.querySelector('.hc-table-wrap')?.scrollLeft||0]));}picker.replaceChildren(new Option('All eligible items',''));inventory.forEach(item=>picker.add(new Option(item.seriesName+' — '+item.episode,item.itemId)));if(inventory.some(item=>item.itemId===selectedItem))picker.value=selectedItem;if(!inventory.length){empty(target,'No cache candidates yet. Start or resume an episode, then refresh.');viewState.inventoryRendered=true;return;}const groups=new Map();inventory.forEach(item=>{if(!groups.has(item.seriesName))groups.set(item.seriesName,[]);groups.get(item.seriesName).push(item);});const renderedSeries=Array.from(groups.entries()).sort(([left],[right])=>left.localeCompare(right)).map(([series,items],index)=>{const details=element('details','hc-series');details.dataset.series=series;details.open=viewState.inventoryRendered?viewState.openSeries.has(series):index===0;const summary=document.createElement('summary'),summaryText=element('strong','',series),total=items.reduce((sum,item)=>sum+item.sizeBytes,0);summary.append(summaryText,element('span','hc-series-count',items.length+' '+(items.length===1?'episode':'episodes')+' · '+formatBytes(total)));const body=document.createElement('div');table(body,['Episode','State','Interest','Users','Priority','Size','Backend','Updated'],items.map(item=>[item.episode,stateBadge(item.state),item.reason||'—',String(item.interestedUsers),String(item.priority),formatBytes(item.sizeBytes),item.backend||'—',formatDate(item.updatedAtUtc)]));details.append(summary,body);return details;});target.replaceChildren(...renderedSeries);renderedSeries.forEach(details=>{const tableWrap=details.querySelector('.hc-table-wrap');if(tableWrap)tableWrap.scrollLeft=viewState.scrollBySeries.get(details.dataset.series)||0;});viewState.inventoryRendered=true;};
